...
for i = 1:n
    ...
    obs(i).uTC = data(i,2); % TC error is in the 2nd column, 1 sigma
    obs(i).TA = data(i,3); % TA (umol/kg) is in the 3rd column
    obs(i).uTA = data(i,4); % TA error is in the 4th column, 1 sigma
    obs(i).sal = data(i,5); % salinity (PSU) 5th column
    obs(i).usal = data(i,6); % salinity error 6th column, 1 sigma

    % temperature and pressure DEpendent 1st system
    obs(i).tp(1).T = 25.0 ; % temperature for ph in Celsius
    obs(i).tp(1).uT = data(i,8); % ph temperature error, 1 sigma
    obs(i).tp(1).P = data(i,9); % pressure (dbar)
    obs(i).tp(1).uP = data(i,10); % pressure error, 1 sigma
    obs(i).tp(1).ph = data(i,11); % ph measured on total scale
    obs(i).tp(1).uph = data(i,12); % ph uncertainty, 1 sigma

    % temperature and pressure DEpendend 2nd system
    obs(i).tp(2).T = 20.0; % temperature for pCO2 in Celsius
    obs(i).tp(2).uT = data(i,13); % pCO2 temperature error, 1 sigma
    obs(i).tp(2).P = 0.0; % pressure in dbar for pCO2
    obs(i).tp(2).uP = 0.003; % dbar, pressure for pCO2 error, 1 sigma
    obs(i).tp(2).pco2 = data(i,14); % partial pressure CO2 (pCO2) (uatm)
    obs(i).tp(2).upco2 = data(i,15); % pCO2 error (uatm), 1 sigma
end
...
% compare(obs,est,opt,1,1,1,'compare.csv'); % to compare to TC;TA pair
% compare(obs,est,opt,1,2,3,'compare.csv'); % to compare to TC;ph pair
% compare(obs,est,opt,2,2,4,'compare.csv'); % to compare to TC;pCO2 pair
